...
const getButtonStyles = (
  ...
  return css`
    ...
    border: ${variant === 'secondary'
      ? `2px solid ${buttonState.border || '#018762'}`
      : 'none'};
    outline: ${buttonState.outline
      ? `2px solid ${buttonState.outline}`
      : 'none'};
    outline-offset: 2px;

    &:hover {
      background-color: ${theme.components.button.states[variant].hover
        .background};
      color: ${theme.components.button.states[variant].hover.text};
      border-color: ${variant === 'secondary'
        ? (theme.components.button.states[variant].hover as ButtonStateStyle)
            .border || '#007756'
        : 'transparent'};
    }

    &:active {
      background-color: ${theme.components.button.states[variant].pressed
        .background};
      color: ${theme.components.button.states[variant].pressed.text};
      border-color: ${variant === 'secondary'
        ? (theme.components.button.states[variant].pressed as ButtonStateStyle)
            .border || '#014C37'
        : 'transparent'};
    }

    &:focus-visible {
      background-color: ${theme.components.button.states[variant].focus
        .background};
      color: ${theme.components.button.states[variant].focus.text};
      border-color: ${variant === 'secondary'
        ? (theme.components.button.states[variant].focus as ButtonStateStyle)
            .border || '#018762'
        : 'transparent'};
      outline: 2px solid
        ${(theme.components.button.states[variant].focus as ButtonStateStyle)
    ...
    &:disabled {
      background-color: ${theme.components.button.states[variant].disabled
        .background};
      color: ${theme.components.button.states[variant].disabled.text};
      border-color: ${variant === 'secondary'
        ? (theme.components.button.states[variant].disabled as ButtonStateStyle)
            .border || '#D1D5DB'
        : 'transparent'};
      cursor: not-allowed;
      opacity: 0.6;
